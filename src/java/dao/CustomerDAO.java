...
public class CustomerDAO {
    ...
    public List<Customer> getCustomersByPage(int page, int recordsPerPage) {
        ...
        String query = "SELECT * FROM customer ORDER BY ID DESC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY";
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setInt(1, start);
    ...
    public List<Customer> searchCustomers(String keyword) {
        List<Customer> customers = new ArrayList<>();
        String query = "SELECT * FROM customer WHERE CustomerName LIKE ? OR Phone LIKE ?";
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
    ...
    public void addCustomer(Customer customer) {
        String query = "INSERT INTO customer (CustomerName, Phone, Address, Points) VALUES (?, ?, ?, ?)";
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
    ...
    public void updateCustomer(Customer customer) {
        String query = "UPDATE customer SET CustomerName = ?, Phone = ?, Address = ?, Points = ? WHERE ID = ?";
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setString(1, customer.getCustomerName());
            stmt.setString(2, customer.getPhone());
            stmt.setString(3, customer.getAddress());
